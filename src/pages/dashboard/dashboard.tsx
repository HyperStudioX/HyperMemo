...
export default function DashboardApp() {
    ...
    if (!user) {
        return (
            <div className="flex h-screen w-screen items-center justify-center bg-bg-subtle relative overflow-hidden">
                {/* Background decorations */}
                <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-primary/10 rounded-full blur-3xl" />
                <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-accent/10 rounded-full blur-3xl" />

                <div className="relative bg-bg-main border border-border rounded-2xl shadow-lg p-10 max-w-[420px] w-full text-center">
                    <div className="mb-6 relative">
                        <div className="absolute inset-0 bg-gradient-to-br from-primary/20 to-accent/10 rounded-full blur-2xl scale-150 opacity-50" />
                        <div className="relative w-24 h-24 mx-auto rounded-2xl bg-gradient-to-br from-primary/10 to-accent/10 border border-primary/20 flex items-center justify-center shadow-lg">
                            <BrainIcon size={48} className="text-primary" />
                        </div>
                    </div>
                    <h1 className="text-3xl font-bold mb-2 text-text-primary font-display">{t('app.signInTitle')}</h1>
                    <p className="text-text-secondary mb-8 leading-relaxed">{t('app.signInDesc')}</p>
                    <button
                        type="button"
                        className="w-full flex items-center justify-center gap-3 px-6 py-3.5 bg-bg-main border border-border rounded-xl text-text-primary font-medium hover:bg-bg-subtle hover:border-primary/30 hover:shadow-md transition-all"
                        onClick={login}
                    >
                    ...
                    <p className="text-xs text-text-secondary mt-6">{t('app.tagline', 'Your second brain for the web')}</p>
                </div>
            </div>
        );
    }
            ...
            `}>
                <div className="p-4 border-b border-border flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-xl font-semibold tracking-tight font-display">{t('app.name')}</h1>
                    </div>
                    <button
                        type="button"
                        onClick={() => setMobileSidebarOpen(false)}
                ...
                {sidebarTab === 'bookmarks' && (
                    <>
                        <div className="px-3 py-2.5 border-b border-border bg-bg-subtle overflow-x-auto scrollbar-hide">
                                ...
                                {availableTags.map(tag => (
                                    ...
                                    >
                                        ...
                                        {tag.bookmarkCount !== undefined && (
                                            <span className={`text-[10px] ${selectedTag === tag.name ? 'text-white/70' : 'text-text-secondary/70'}`}>
                                                {tag.bookmarkCount}
                                            </span>
                                        )}
                                    </button>
                        ...
                        <div className="flex-1 overflow-y-auto p-2 flex flex-col gap-1">
                            {filteredBookmarks.map((bookmark, index) => {
                                ...
                                return (
                                    <div
                                        key={bookmark.id}
                                        className={`stagger-item flex gap-3 p-3 rounded-lg cursor-pointer border transition-all ${activeBookmarkId === bookmark.id ? 'bg-bg-main border-primary shadow-sm' : 'border-transparent hover:bg-bg-main hover:border-border'}`}
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0 overflow-hidden">
                                            <div className="flex items-center gap-1.5 mb-1">
                                                <h3 className="text-[0.9375rem] font-medium line-clamp-2 leading-snug flex-1">{bookmark.title || t('dashboard.untitled')}</h3>
                                                {hasAiContent && (
                                                    <span className="shrink-0 w-4 h-4 rounded-full bg-accent/20 flex items-center justify-center" title="AI enhanced">
                    ...
                    {activeTab === 'overview' && (
                        activeBookmark ? (
                            ...
                            <div className="flex items-center justify-center h-full">
                                <div className="flex flex-col items-center gap-8 max-w-[640px] w-full mx-auto px-4">
                                    {/* Hero Section with texture */}
                                    <div className="relative">
                                        <div className="absolute inset-0 bg-gradient-to-br from-primary/20 via-accent/10 to-transparent rounded-full blur-3xl scale-150 opacity-50" />
                                        <div className="relative flex flex-col items-center">
                                            <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-primary/10 to-accent/10 border border-primary/20 flex items-center justify-center mb-6 shadow-lg animate-pulse-glow">
                                                <BrainIcon size={48} className="text-primary" />
                                            </div>
                                            <h1 className="text-4xl font-bold tracking-tight mb-3 font-display text-center bg-gradient-to-r from-text-primary via-primary to-accent bg-clip-text text-transparent">
                                                {t('app.name')}
                                            </h1>
                                            <p className="text-lg text-text-secondary text-center max-w-md leading-relaxed">
                                                {t('app.slogan')}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap justify-center gap-3 text-sm">
                                        <div className="flex items-center gap-2 px-3 py-1.5 bg-bg-subtle rounded-full text-text-secondary">
                                            ...
                                            <span>{t('dashboard.selectBookmark')}</span>
                                        </div>
                                        <div className="flex items-center gap-2 px-3 py-1.5 bg-bg-subtle rounded-full text-text-secondary">
                                ...
                                {!messages.length && (
                                    <div className="flex flex-col items-center justify-center h-full text-center px-4">
                                        <div className="relative mb-6">
                                            <div className="absolute inset-0 bg-gradient-to-br from-primary/30 to-accent/20 rounded-full blur-2xl scale-150 opacity-40" />
                                            <div className="relative w-20 h-20 rounded-2xl bg-gradient-to-br from-primary/10 to-accent/5 border border-primary/20 flex items-center justify-center shadow-lg">
                                                <BrainIcon size={40} className="text-primary" />
                                            </div>
                                        </div>
                                        <h3 className="text-2xl md:text-3xl font-semibold mb-3 text-text-primary font-display">{t('chat.welcomeTitle')}</h3>
                                        <p className="text-base md:text-lg text-text-secondary max-w-md leading-relaxed">{t('chat.welcomeSubtitle')}</p>
                                        <div className="flex flex-wrap justify-center gap-2 mt-6">
                                            <span className="px-3 py-1.5 text-xs bg-primary/10 text-primary rounded-full font-medium">
                                                {t('chat.tipAsk', 'Ask about your bookmarks')}
                                            </span>
                                            <span className="px-3 py-1.5 text-xs bg-accent/10 text-accent rounded-full font-medium">
                                                {t('chat.tipSearch', 'Search your knowledge')}
                                            </span>
                                        </div>
                                    </div>
                                )}
